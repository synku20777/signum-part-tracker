...
async def test_authentication(ebay_adapter):
    respx.post(EBAY_AUTH_URL).mock(return_value=httpx.Response(200, json={"access_token": "test_token"}))
...
async def test_search_returns_listings(ebay_adapter):
    ...
    results = await ebay_adapter.search(["i3401009"])
...
async def test_pagination(ebay_adapter):
    ...
    resp_page_last["itemSummaries"] = [resp_page_last["itemSummaries"][0].copy() for _ in range(10)]
    for i, item in enumerate(resp_page_last["itemSummaries"]):
        item["itemId"] = f"v1|last{i}|0"

    respx.get(EBAY_SEARCH_URL).mock(side_effect=[
...
async def test_rate_limit_retry(ebay_adapter):
    ...
    respx.get(EBAY_SEARCH_URL).mock(side_effect=[
        httpx.Response(429),
        httpx.Response(200, json=get_sample_response())
    ])

    results = await ebay_adapter.search(["test"])
    assert len(results) == 1
...
async def test_empty_results(ebay_adapter):
    respx.post(EBAY_AUTH_URL).mock(return_value=httpx.Response(200, json={"access_token": "test_token"}))
    respx.get(EBAY_SEARCH_URL).mock(return_value=httpx.Response(200, json={"itemSummaries": [], "total": 0, "limit": 50, "offset": 0}))
...
async def test_deduplication_across_queries(ebay_adapter):
    ...
    results = await ebay_adapter.search(["query1", "query2"])
